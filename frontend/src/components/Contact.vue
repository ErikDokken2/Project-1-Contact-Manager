  ...
  <div>
    ...
    <tbody>
      ...
      <tr v-for="(user, index) in (searchResults.length > 0 ? searchResults.slice(min * 10, maxInd * page) : filteredPeople.slice(min * 10, maxInd * page))" :key="index">
        ...
        <td>{{user.birthday}}</td>
        <td><button type="button" class="btn btn-warning" id="floated" data-bs-toggle="modal" data-bs-target="#exampleModal2" v-on:click="edit_id = user.id; edit_first = user.first; edit_birthday=user.birthday; edit_email=user.email; edit_phone_number=user.phone_number;edit_last=user.last">Edit</button></td>
...
try {
  /*const response = await axios.post('search.php', {
...
*/
      ...
      if(!isError) {
            ...
            this.new_id = max + 1
            this.users.push({ id: this.new_id, first: this.new_first, last: this.new_last,  email: this.new_email, birthday: this.new_birthday, phone_number:this.new_phone_number, isFavorites:false })
            //  let i = 0
            //   for (i; i < this.users.length+1; i++) {
           
              /* not the right api call
...
this.users = this.users.map(person => {
  if (person.email === email) {
    ...
    axios.post('update.php', {
      firstName: updatedPerson.first,
      lastName: updatedPerson.last,
      email: updatedPerson.email,
      phoneNumber: updatedPerson.phone_number,
      streetAddress: '',
      city: '',
      state: '',
      zip: '',
  ...
  return person;
      ...
      async deleteData (userid) {
        ...
        try {
          ...
          //proper delete 
          /*const response = await axios.post('delete.php', {
          userId: userId,//the user
